...
export const createDatabaseIfNotExists = async (): Promise<void> => {
    ...
    try {
        ...
        const db: QueryResult<unknown[]> = await pool.query(`SELECT datname FROM pg_catalog.pg_database WHERE datname = '${dbName}'`);
        if (db.rows.length) {
...
export const createSchemaIfNotExists = async (): Promise<void> => {
    ...
    try {
        const existingSchema: unknown[] = await database.query(`SELECT schema_name FROM information_schema.schemata WHERE schema_name = '${schemaName}'`);
