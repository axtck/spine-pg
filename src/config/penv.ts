...
if (!isOfEnum(process.env.NODE_ENV, Constants.environmentEnumValues)) {
  throw new Error(
    `environment '${process.env.NODE_ENV}' invalid, possible environments: undefined (development) - development - test - staging - production`
  );
}
...
export const penv = {
  ...
  db: {
    ...
    pgSchema: "spine" // static name for initializing (if change => also change in database/initialize.sql)
  },
  auth: {
